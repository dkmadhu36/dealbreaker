...
export type CardType = 'property' | 'action' | 'money' | 'rent' | 'wildcard';

export type ActionType =
  | 'dealBreaker' | 'slyDeal' | 'forcedDeal'
  | 'justSayNo' | 'debtCollector' | 'birthday'
  | 'passGo' | 'house' | 'hotel' | 'doubleRent' | 'rent';
...
export const ACTION_CARD_ICONS: Record<ActionType, string> = {
  dealBreaker: '🤝',
  slyDeal: '🦊',
  forcedDeal: '⇄',
  justSayNo: '✋',
...
export const CARDS_MAP: Record<string, Card> = CARDS_DATA.reduce((acc, card) => {
  acc[card.id] = card;
  return acc;
}, {} as Record<string, Card>);

export function getCardById(cardId: string): Card | undefined {
  return CARDS_MAP[cardId];
}
